...
CDDAManager::CDDAManager( const HINSTANCE instance, const HWND hwnd, Library& library, Handlers& handlers, MusicBrainz& musicbrainz ) :
	m_Library( library ),
...
void CDDAManager::UpdateThreadHandler()
{
	...
	while ( WaitForMultipleObjects( 2, eventHandles, FALSE /*waitAll*/, INFINITE ) != WAIT_OBJECT_0 ) {
		...
		for ( const wchar_t drive : drives ) {
			try {
				CDDAMedia cddaMedia( drive, m_Library, m_MusicBrainz );
				std::lock_guard<std::mutex> lock( m_MediaMutex );
