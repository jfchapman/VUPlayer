...
WndToolbarEQ::WndToolbarEQ( HINSTANCE instance, HWND parent ) :
	WndToolbar( instance, parent, ID_TOOLBAR_EQ ),
	m_ImageList( nullptr )
{
	CreateButtons();
...
void WndToolbarEQ::CreateImageList()
{
	...
	HICON hIcon = static_cast<HICON>( LoadImage( GetInstanceHandle(), MAKEINTRESOURCE( IDI_EQ ), IMAGE_ICON, cx, cy, LR_DEFAULTCOLOR | LR_SHARED ) );
	if ( NULL != hIcon ) {
